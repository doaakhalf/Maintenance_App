...
@section('title', 'Edit Calibration Perform')

@section('content_header')
<h1>Edit Calibration Perform</h1>
@stop

@section('content')
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.calibration-perform.update', $calibrationPerform->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="row">
                <!-- Calibration Request ID -->
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="calibration_request_id">Calibration Request ID</label>
                        <input type="text" id="calibration_request_id" name="calibration_request_id" class="form-control" value="{{ old('calibration_request_id', $calibrationPerform->calibration_request_id) }}" readonly>
                    </div>
                </div>
                <!-- Date Performed -->
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="date" id="perform_date" name="perform_date" class="form-control" value="{{ old('perform_date', date('Y-m-d',strtotime($calibrationPerform->perform_date))) }}">
                    </div>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea id="service_report" name="service_report" class="form-control">{{ old('service_report', $calibrationPerform->service_report) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div id="spare-parts-container">
                @foreach ($calibrationPerform->performDetails as $detail)
            ...
            <!-- Submit Button -->
            <button type="submit" class="btn btn-primary">Update Calibration Perform</button>
        </form>
    </div>
</div>
@stop
